...
void Raytracer::CreateAll()
{
	...
	scene.CreateScene(primitive_head);
...
void Raytracer::MultiThreadFuncCalColorInRange(int i, int** sample, int w1, int w2)
{
	srand(i);
	Vector3 ray_O = camera->GetO();
	for (int j = w1; j < w2; j++) {
...
void Raytracer::MultiThreadFuncResamplingInRange(int i, int** sample, int w1, int w2)
{
	Vector3 ray_O = camera->GetO();
	int H = camera->GetH(), W = camera->GetW();
	for (int j = w1; j < w2; j++) {
...
void Raytracer::MultiThreadRunDebugRun(int w1, int w2, int h1, int h2, bool resampling)
{
	CreateAll();
	...
	std::vector<std::thread> threads(h1 - h2);
	for (int i = h2; i < h1; i++)
	{
		threads[i - h2] = std::thread(&Raytracer::MultiThreadFuncCalColorInRange, this, i, sample, w1, w2);
	}
	for (int i = h2; i < h1; i++)
	{
		threads[i - h2].join();
	...
	if (resampling) {
		for (int i = h2; i < h1; i++)
		{
			threads[i - h2] = std::thread(&Raytracer::MultiThreadFuncResamplingInRange, this, i, sample, w1, w2);
		}
		for (int i = h2; i < h1; i++)
		{
			threads[i - h2].join();
